...
export async function classifyScamTypeAndTitle(content: string, retry?: boolean): Promise<{
  ...
  try {
    ...
    if (!retry && error.message.includes("429 Rate limit")){
      logger.error(`分類失敗: ${error instanceof Error ? error.message : String(error)}`);
      logger.error(`等待 10 秒後重試`);
      await new Promise((resolve) => setTimeout(resolve, 10000));
      return classifyScamTypeAndTitle(content, true);
    }
    else{
      logger.error(`分類失敗: ${error instanceof Error ? error.message : String(error)}`);
      return {
